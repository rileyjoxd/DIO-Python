...
async def decode_jwt(token: str) -> JWTToken | None:
    try:
        decoded_token = decode(token, SECRET, audience="curso-fastapi", algorithms=[ALGORITHM])
...
async def get_current_user(credentials: Annotated[HTTPAuthorizationCredentials, Depends(bearer_scheme)]) -> dict[str, int]:
    token = credentials.credentials
    payload = await decode_jwt(token)
    if not payload:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or expired token.")
    decoded = decode(token, SECRET, audience="curso-fastapi", algorithms=[ALGORITHM])
